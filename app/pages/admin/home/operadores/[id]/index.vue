...
const deleteOperador = async () => {
  try {
    ...
    toast.add({
      ...
      detail: 'Caso esteja autenticado, será redirecionado para a página de login.',
      life: 3000,
    });
...
const confirmLoginOperador = (event: any) => {
  confirm.require({
    target: event.currentTarget,
    message: 'Isso encerrará a sessão atual de outras pessoas conectadas ao operador.',
    icon: 'pi pi-exclamation-triangle',
    rejectProps: {
      label: 'Cancelar',
      severity: 'secondary',
  ...
  <div class="page-content">
    ...
    <main class="container mx-auto flex h-full flex-col p-4">
      ...
      >
        ...
        <!-- Label -->
        <div class="flex items-center px-2">
          <span class="w-64 text-surface-600 dark:text-surface-400">Descrição</span>
          <span class="flex-1 text-surface-900 dark:text-surface-0">
            <input
              ref="inputLabelRef"
