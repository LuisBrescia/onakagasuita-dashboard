...
const handleSaveHorarioInicio = async () => {
  const salaoRequest: Partial<SalaoRequest> = {
    horario_funcionamento_inicio: formatarHorario(formData.value.horarioFuncionamentoInicio),
  };
  await updateSalao(Number(route.params.id), salaoRequest);
  originalData.value.horarioFuncionamentoInicio = formData.value.horarioFuncionamentoInicio;
  isEditing.value.horarioFuncionamentoInicio = false;
};

const handleCancelHorarioInicio = () => {
  formData.value.horarioFuncionamentoInicio = originalData.value.horarioFuncionamentoInicio;
  isEditing.value.horarioFuncionamentoInicio = false;
};
...
const handleSaveHorarioFinal = async () => {
  const salaoRequest: Partial<SalaoRequest> = {
    horario_funcionamento_fim: formatarHorario(formData.value.horarioFuncionamentoFinal),
  };
  await updateSalao(Number(route.params.id), salaoRequest);
  originalData.value.horarioFuncionamentoFinal = formData.value.horarioFuncionamentoFinal;
  isEditing.value.horarioFuncionamentoFinal = false;
};

const handleCancelHorarioFinal = () => {
  formData.value.horarioFuncionamentoFinal = originalData.value.horarioFuncionamentoFinal;
  isEditing.value.horarioFuncionamentoFinal = false;
};
...
const getSalao = async () => {
  ...
  formData.value = {
    nome: salao.nome,
    ativo: salao.ativo,
    horarioFuncionamentoInicio: converterHoraParaISO(salao.horario_funcionamento_inicio),
    horarioFuncionamentoFinal: converterHoraParaISO(salao.horario_funcionamento_fim),
  ...
  <div class="page-content">
    <TheTopbar>
      <template #actions>
        ...
        <Button
          ...
          size="small"
          severity="info"
          @click="router.push(`/admin/home/estrutura/${route.params.id}/layout`)"
        />
    ...
    <main class="page-inner container mx-auto flex h-full flex-col">
      ...
      >
        ...
        <div class="flex items-center px-2">
          <span class="w-64 text-surface-600 dark:text-surface-400">Dias de funcionamento</span>
          <span class="flex flex-1 flex-wrap gap-2 text-surface-900 dark:text-surface-0">
