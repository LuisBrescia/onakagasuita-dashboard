...
function onChange(event: Event, changeItems: GridStackNode[]) {
  ...
  changeItems.forEach((item) => {
    var widget = mesasData.value.find((w) => w.id == item.id);
    if (!widget) {
      return;
    }
    widget.x = item.x;
...
function removerMesa(widget: ItemMesa) {
  var index = mesasData.value.findIndex((w) => w.id == widget.id);
...
function updateInfo() {
  color.value =
    grid?.engine.nodes.length == mesasData.value.length ? 'black' : 'red';
